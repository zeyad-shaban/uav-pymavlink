...
def uploadfence(fence_file):
    ...
    fencelist,fenceNo = robenuav.FileList(fence_file)

    fence_list = []
    for i in range (fenceNo+1):
        NA, LatA, LongA = robenuav.Air_Drop_and_fence_Coordinates(i, fencelist) #get coordinates of first waypoint
        fence_list.append((float(LatA),float(LongA)))
    ...
    while True:
        ...
        message = vehicle.recv_match(type="PARAM_VALUE",
                                     blocking=True)
    ...
    while idx < len(fence_list):
        ...
        message = vehicle.recv_match(type="FENCE_POINT",
                                     blocking=True)
