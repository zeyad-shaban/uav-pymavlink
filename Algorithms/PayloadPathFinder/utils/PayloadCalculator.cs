class PayloadCalculator
{
    ...
    private static (double x, double y) PayloadDropEq(double H1, double Vpa, double Vag, double angle)
    {
        ...
        List<double> H = [H1];  // height of the plane in meters
        List<double> ty = [0];  // duration of fall
        List<double> Vy = [0];  // velocity in downward direction
        List<double> acc = [9.81];  // acceleration in downward direction
        List<double> Dy = [0];  // upward drag force
        List<double> dy = new List<double> { 0 };  // deceleration due to drag force
        int k = 1;
        ...
        List<double> Vx = [Vpg];  // velocity of payload in horizontal direction
        List<double> R = [0];  // distance covered by payload in horizontal direction
        List<double> Dx = [Cd * rho * Math.Pow(Vx[0], 2) * A / 2];  // horizontal drag on the payload
        List<double> dx = [Dx[0] / m];  // horizontal deceleration on the payload
        k = 1;
